...
echo -e "Timestamp-Statrun\tTimestamp-Execution\tHost\tTrack-ID\tBenchmark-ID\tSolver-ID\tInstance-ID\tRun-ID\tExit-Code-Solver\tExit-Code-Checker\tCost-Solver\tCost-Checker\tProblem-Type\tWall-Clock-Time\tTime-Grounder\tMemory-Grounder\tTime-Solver\tMemory-Solver\tTime-Combined\tMemory-Combined\tDiscrepancies-Solver"
echo -e "${DATE0}\t${EXEC_TIMESTAMP}\t${EXEC_HOST}\t${TRACK_ID}\t${BENCHMARK_ID}\t${SOLVER_ID}\t${INSTANCE_ID}\t${RUN_ID}\t${EC_SOLVER}\t${EC_CHECKER}\t${COST_SOLVER}\t${COST_CHECKER}\t$PROBLEM_TYPE\t$WALL_TIME_COMBINED_IN_SECS\t${TIME_GROUNDER}\t${MEM_GROUNDER}\t${TIME_SOLVER}\t${MEM_SOLVER}\t${TIME_COMBINED}\t${MEM_COMBINED}\t${DISCREPANCIES_SOLVER}"
exit 3
